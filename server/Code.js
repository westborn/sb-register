this.CONFIG = {
  ...
  SAMPLE_ENTRY: {
    "id": "2710f41b-b5fb-4d07-b774-8ec4090fd072",
    "inOrOut": "Outdoor",
  ...
  SAMPLE_REGISTRATION: {
    id: 'bb655afd-7adb-40ac-8d4b-eeb6063a1dc7',
    firstName: 'll',
    lastName: 'll',
    phone: 'll',
    email: 'ff',
  ...
  SAMPLE_IMAGE: {
    "id": "217f1f16-20f4-4a3a-9a45-2dbdc0673b3f",
    "entryId": "668eb6d8-a764-4993-a198-2db8c522d95b",
...
function doPost(e) {
  ...
  try {
    ...
    switch (requestType) {
      case "getEntry":
        res = getInformationForEmailAddress(request, ss)
        break
      case "createEntry":
        res = createEntry(request.data, ss)
        break
      case "deleteEntry":
        res = deleteEntry(request.data, ss)
        break
      case "modifyEntry":
        res = modifyEntry(request.data, ss)
        break
      case "createRegistration":
        res = createRegistration(request.data, ss)
        break
      case "modifyRegistration":
        res = modifyRegistration(request.data, ss)
        break
      case "createImage":
        res = createImage(request.data, ss)
        break
      default:
...
function createImage(request, ss) {
  // make a google filename artistSurname_title_id
  ...
  if (!request.id || request.id.trim() === "") {
    return sendResponse('error', 'Invalid ID for create image')
  }
  try {
    ...
    // const rowToChange = wbLib.getRowFromColumnSearch(sheet.getDataRange().getValues(), "id", entryId)
    // if (rowToChange) {
...
function modifyRegistration(request, ss) {
  ...
  try {
    ...
    console.log(row)
    const rowToModify = wbLib.getRowFromColumnSearch(sheet.getDataRange().getValues(), "id", request.id)
    if (rowToModify) {
...
function deleteEntry(request, ss) {
  ...
  try {
    const sheet = ss.getSheetByName('Entries')
    const rowToDelete = wbLib.getRowFromColumnSearch(sheet.getDataRange().getValues(), "id", request.id)
    if (rowToDelete) {
...
function getFullRegistration(email, ss) {
  ...
  return { data: { registration: registrationArray[0], entries: [...entryArray] } }
}
